...
**Lighting**: before every AI check, the printer's chamber light is switched on and the check waits ~4.5s for the camera's auto-exposure to settle, so checks always analyze a well-lit frame (the light is left on afterward).
...
### Files to back up
`bambu-maintenance.json`, `bambu-history.json`, `bambu-queue.json`, `bambu-etsy-tokens.json`, `bambu-etsy-mappings.json`, `bambu-ebay-tokens.json`, `bambu-ebay-mappings.json`, `bambu-order-tracking.json`, `bambu-remember-me.json`, `bambu-notification-suppressed.json`, `bambu-ams-dry.json`, `bambu-ams-dry-sessions.json`, `bambu-auto-start.json`, `bambu-auto-queue.json`, `bambu-ai-prompts.json`, `bambu-tasmota-autooff.json`, `bambu-stock.json`, `bambu-bed-reference.json`, the `bambu-bed-refs/` folder, the library folder, and `.env` - or use the Backup button (covers maintenance/history/queue/library, not `.env` or the marketplace token/mapping files).
