...
- **Mappings** - every listing → gcode assignment in one place. For **Etsy**, a button pulls ALL active shop listings into a table (the connect flow's `listings_r` scope covers this) so products can be mapped **before an order ever arrives** - which is exactly what auto-queue needs to handle a first-time order hands-free. For **eBay**, the fulfillment-only API scope can't list inventory, so rows come from saved mappings plus listing keys seen in open orders. A third table lists every raw saved mapping (including per-variation ones from the order pages) with edit/delete. Mappings saved here are listing-wide - order lookups fall back to them for any variation.
